...
<style>
    ...
    /* Sembunyikan header & footer bawaan filament */
    ...
    /* Checkbox label text (Remember me, etc) */
    .fi-simple-main .fi-checkbox-label,
    .fi-simple-main [class*='checkbox'] span,
    .fi-simple-main .fi-fo-field-wrp label[class*='checkbox'],
...
<div id="dineflo-left-panel" style="position:fixed; inset:0; width:50%; height:100vh; display:flex; flex-direction:column; justify-content:space-between; padding:3rem; z-index:10; overflow:hidden; background:#020617;">
    ...
    <div style="position:relative;">
        <h1 style="font-size:clamp(2rem,3.5vw,3rem); font-weight:900; color:#fff; line-height:1.1; letter-spacing:-0.02em; margin-bottom:1.25rem;">
            Mulai hari Anda<br>
            <span style="background:linear-gradient(90deg,#f59e0b,#fde68a); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text;">dengan kendali penuh.</span>
        </h1>
        <p style="font-size:1rem; color:#64748b; line-height:1.7; max-width:22rem;">
            Akses Dashboard HQ, Kitchen Display, dan Sistem POS dalam satu platform operasional terintegrasi.
        </p>
    </div>

    {{-- Testimonial --}}
    <div style="position:relative; background:rgba(255,255,255,0.025); border:1px solid rgba(255,255,255,0.07); border-radius:1.25rem; padding:1.5rem; backdrop-filter:blur(12px);">
        <div style="display:flex; gap:0.25rem; margin-bottom:0.75rem;">
            @for($i=0; $i<5; $i++)
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="#f59e0b" style="width:1rem; height:1rem;">
                <path fill-rule="evenodd" d="M10.868 2.884c-.321-.772-1.415-.772-1.736 0l-1.83 4.401-4.753.381c-.833.067-1.171 1.107-.536 1.651l3.62 3.102-1.106 4.637c-.194.813.691 1.456 1.405 1.02L10 15.591l4.069 2.485c.713.436 1.598-.207 1.404-1.02l-1.106-4.637 3.62-3.102c.635-.544.297-1.584-.536-1.65l-4.752-.382-1.831-4.401Z" clip-rule="evenodd" />
            </svg>
            @endfor
        </div>
        <p style="color:#cbd5e1; font-size:0.9rem; line-height:1.7; font-style:italic; margin-bottom:1rem;">
            "Sistem kasir paling mulus yang pernah kami pakai. QR Order dan KDS-nya memangkas waktu tunggu tamu secara drastis."
        </p>
        <div style="display:flex; align-items:center; gap:0.75rem;">
            <div style="width:2.25rem; height:2.25rem; border-radius:50%; background:linear-gradient(135deg,#6366f1,#a855f7); display:flex; align-items:center; justify-content:center; font-weight:700; color:#fff; font-size:0.8rem;">AM</div>
            <div>
                <div style="color:#fff; font-weight:700; font-size:0.875rem;">Andi M.</div>
                <div style="color:#475569; font-size:0.75rem;">Owner, Kopi Senja</div>
            </div>
        </div>
    </div>
</div>

{{-- ===== PILL TOGGLE (top-right, di atas right panel) ===== --}}
<div style="position:fixed; top:1.5rem; right:1.75rem; z-index:50;">
    <div style="display:flex; align-items:center; padding:0.25rem; border-radius:9999px; background:rgba(9,14,29,0.95); border:1px solid rgba(255,255,255,0.08); backdrop-filter:blur(12px);">
        ...
        <a href="{{ filament()->getLoginUrl() }}" style="display:flex; align-items:center; gap:0.375rem; padding:0.5rem 1.25rem; border-radius:9999px; background:linear-gradient(135deg,#f59e0b,#d97706); color:#fff; font-weight:700; font-size:0.8125rem; text-decoration:none; box-shadow:0 2px 12px rgba(245,158,11,0.4);">
            <svg style="width:0.875rem; height:0.875rem;" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6a2.25 2.25 0 0 0-2.25 2.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15m3 0 3-3m0 0-3-3m3 3H9"/></svg>
            Login
        </a>
        @if(filament()->hasRegistration())
        <a href="{{ filament()->getRegistrationUrl() }}" style="padding:0.5rem 1.25rem; border-radius:9999px; color:#64748b; font-weight:600; font-size:0.8125rem; text-decoration:none; transition:color 0.2s;" onmouseover="this.style.color='#fff'" onmouseout="this.style.color='#64748b'">
            Daftar
        </a>
        @endif
    </div>
...
{{-- ===== FORM HEADER (masuk ke dalam fi-simple-main via slot) ===== --}}
<div style="margin-bottom:2rem;">
    <p style="font-size:0.7rem; font-weight:700; letter-spacing:0.1em; color:#f59e0b; text-transform:uppercase; margin-bottom:0.5rem;">Panel Restoran</p>
    <h2 style="font-size:1.875rem; font-weight:900; color:#fff; letter-spacing:-0.025em; margin-bottom:0.5rem;">Selamat Datang Kembali</h2>
    <p style="font-size:0.875rem; color:#64748b; line-height:1.6;">Masukkan email &amp; password Anda untuk melanjutkan.</p>
</div>
