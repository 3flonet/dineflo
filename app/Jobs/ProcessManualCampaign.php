...
class ProcessManualCampaign implements ShouldQueue
{
    ...
    {
        // 1. Update Status to Sending immediately to prevent double processing
        ...
        // 2. Query Recipients based on segmentation
        ...
        // Add more segmentation logic here if needed (activity, etc)
        ...
        foreach ($members as $member) {
            $jobs[] = new SendCampaignEmailJob($this->campaign, $member);
        }

        // 4. Dispatch Batch
        $campaignId = $this->campaign->id;
        Bus::batch($jobs)
            ->name('Broadcast Campaign: ' . $this->campaign->name)
            ->then(function () use ($campaignId) {
                EmailCampaign::find($campaignId)->update(['status' => 'completed']);
            })
